...
def list_unspents(wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
    wallet, online,xpub = wallet_dep
    unspents = wallet.list_unspents(online, False, False)
    return unspents

@router.post("/wallet/createutxosbegin",response_model=str)
def create_utxos_begin(req: CreateUtxosBegin, wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
    wallet, online,xpub = wallet_dep
...
def create_utxos_end(req: CreateUtxosEnd, wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
    wallet, online,xpub = wallet_dep
...
def get_btc_balance(wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
    wallet, online,xpub = wallet_dep
    btc_balance = wallet.get_btc_balance(online, False)
    return btc_balance

@router.post("/wallet/address",response_model=str)
def get_address(wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
    wallet,online, xpub= wallet_dep
    address = wallet.get_address()
    return address
...
def get_asset_balance(req: AssetBalanceRequest, wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
...
def generate_invoice(req: RgbInvoiceRequestModel, wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
    wallet, online,xpub = wallet_dep
    receive = wallet.blind_receive(req.asset_id, req.amount, 3600, [PROXY_URL], 1)
    return receive

@router.post("/wallet/failtransfers")
def failtransfers(req: FailTransferRequestModel, wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
    wallet, online,xpub = wallet_dep
...
def list_transaction(wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
    wallet, online ,xpub= wallet_dep
...
def refresh_wallet(wallet_dep: tuple[Wallet, object,str]=Depends(get_wallet)):
    wallet, online,xpub = wallet_dep
